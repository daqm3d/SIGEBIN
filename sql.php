...
/*************************/  
  ...
  function  find_by_departures($table){
   global $db;
   if(tableExists($table)){
       $sql = find_by_sql("SELECT t1.name as name1,t1.quantity,t1.serial,t1.model,t1.bien, t2.name FROM products t1 INNER JOIN categories t2 ON t1.categorie_id=t2.id WHERE t2.name='". $_GET["s_emp"] ."' ORDER by categorie_id desc;");
       return $sql;
...
/*********************************************************/
/*function for perform commands centers queries**********/
/*******************************************************/
function find_center($table) {
   global $db;
   if(tableExists($table))
   {
     return find_by_sql("SELECT * FROM ".$db->escape($table));
...
/*--------------------------------------------------------------*/
 ...
 function updateLastLogIn($user_id)
  ...
  function join_product_table(){
     global $db;
    $sql  =" SELECT p.id,p.name,p.quantity,p.marca,p.serial,p.model,p.date,c.name";
    $sql  .=" AS categorie";
    $sql  .=" FROM products p";
 ...
 function find_recent_product_added($limit){
   global $db;
   $sql   = " SELECT p.id,p.name,p.marca,p.categorie_id FROM products p";
   //$sql  .= " AS categorie FROM products p";
   //$sql  .= " LEFT JOIN categories p ON c.id = p.categorie_id";
   //$sql  .= " LEFT JOIN media m ON m.id = p.media_id";
